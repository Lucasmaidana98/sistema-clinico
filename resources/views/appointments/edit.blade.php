...
@section('title', 'Editar Cita - Sistema Clínico')
    ...
    <div class="bg-white rounded-lg shadow-sm">
        <div class="p-6 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Editar Cita</h1>
                    <p class="text-gray-600">Modifica la información de la cita</p>
                </div>
                <div class="flex space-x-2">
                    <a href="{{ route('appointments.show', $appointment) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
        ...
        <form method="POST" action="{{ route('appointments.update', $appointment) }}" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="appointment_type" 
                            ...
                            <option value="consulta_general" {{ old('appointment_type', $appointment->appointment_type) === 'consulta_general' ? 'selected' : '' }}>Consulta General</option>
                            <option value="control" {{ old('appointment_type', $appointment->appointment_type) === 'control' ? 'selected' : '' }}>Control</option>
                            <option value="emergencia" {{ old('appointment_type', $appointment->appointment_type) === 'emergencia' ? 'selected' : '' }}>Emergencia</option>
                            <option value="revision" {{ old('appointment_type', $appointment->appointment_type) === 'revision' ? 'selected' : '' }}>Revisión</option>
                            <option value="especialista" {{ old('appointment_type', $appointment->appointment_type) === 'especialista' ? 'selected' : '' }}>Especialista</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select id="duration_minutes" 
                            ...
                            <option value="15" {{ old('duration_minutes', $appointment->duration_minutes) == '15' ? 'selected' : '' }}>15 minutos</option>
                            <option value="30" {{ old('duration_minutes', $appointment->duration_minutes) == '30' ? 'selected' : '' }}>30 minutos</option>
                            <option value="45" {{ old('duration_minutes', $appointment->duration_minutes) == '45' ? 'selected' : '' }}>45 minutos</option>
                            <option value="60" {{ old('duration_minutes', $appointment->duration_minutes) == '60' ? 'selected' : '' }}>1 hora</option>
                            <option value="90" {{ old('duration_minutes', $appointment->duration_minutes) == '90' ? 'selected' : '' }}>1 hora 30 minutos</option>
                            <option value="120" {{ old('duration_minutes', $appointment->duration_minutes) == '120' ? 'selected' : '' }}>2 horas</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="reason" 
                                  ...
                                  placeholder="Describe el motivo de la cita">{{ old('reason', $appointment->reason) }}</textarea>
                        @error('reason')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                <a href="{{ route('appointments.show', $appointment) }}" class="px-6 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 transition-colors">
                    Cancelar
                </a>
